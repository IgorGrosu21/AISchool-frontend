...
export function createServiceClient(config: ServiceConfig): ServiceClient {
  ...
  const request: RequestFn = async (
    { requiresAuth = true, ...requestConfig },
    customToken,
  ) => {
  ...
  const deleteDetailedFile: DeleteDetailedFileFn = async (
    requestConfig,
    id,
  ) => {
    return deleteFile({
      ...requestConfig,
      params: { ...requestConfig.params, id },
    });
  };
