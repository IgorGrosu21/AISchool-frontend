 ...
 * 
 */
export const createFileViewset = <RouteTypes extends RouteTypeRegistry, N extends RouteName<RouteTypes>>(
  ...
  return {
    send: async args => config.client.sendDetailedFile(
      resolveRequestConfig({...args, type: 'sendDetailedFile'})
    ),
    delete: async ({id, ...args}) => config.client.deleteDetailedFile(
