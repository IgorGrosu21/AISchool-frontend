 ...
 *
 */
export const createDefaultViewset = <
...
>(
  ...
  const checkMethod = (method: (typeof config.supportedMethods)[number]) => {
    ...
    return async () => {
      throw new Error(
        `Method '${method}' is not supported for this viewset. Supported methods: ${config.supportedMethods.join(", ")}`,
      );
    };
  };
  ...
  return {
    ...
    delete:
      checkMethod("delete") ??
      (async (args) =>
        config.client.deleteFile(
          resolveRequestConfig({ ...args, type: "delete" }),
        )),
    sendFile:
      checkMethod("sendFile") ??
      (async (args) =>
        config.client.sendFile(
          resolveRequestConfig({ ...args, type: "sendFile" }),
        )),
    deleteFile:
      checkMethod("deleteFile") ??
      (async (args) =>
        config.client.deleteFile(
