...
export function Content({ translations }: { translations: string }) {
  const t = useTranslations(`animated_pages.restricted.${translations}`);
  const Feature1Icon = useMemo(
    () => (translations === "ad" ? StarIcon : ScheduleIcon),
    [translations],
  );
  const Feature2Icon = useMemo(
    () => (translations === "ad" ? RocketLaunchIcon : ConstructionIcon),
    [translations],
  );
      ...
      >
        ...
        <Fade in={isMounted} timeout={1600}>
          <Stack direction={{ xs: "column", sm: "row" }} gap={2} sx={{ mt: 2 }}>
            ...
            >
              <Feature1Icon color="primary" sx={{ fontSize: "2rem", mb: 1 }} />
              <Typography
                variant="h6"
                color="primary"
        ...
        <Fade in={isMounted} timeout={2400}>
          <Link href={translations === "ad" ? "/subscriptions" : "/core"}>
            <Button
              variant="contained"
              size="large"
              color="primary"
