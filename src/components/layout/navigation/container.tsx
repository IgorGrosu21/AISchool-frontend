...
export async function NavigationContainer({segments = [], last, children}: NavigationContainerProps) {
  const t = await getTranslations('layout.edit');
  ...
  return <Stack gap={{ xs: 4, md: 8 }} sx={{flex: 1, px: { xs: 2, sm: 4, md: 8, lg: 16 }, py: { xs: 4, md: 8 }}}>
    ...
    <AdUnit slot="8181166943" style={{height: '100px'}} />
    <Stack gap={{ xs: 4, md: 8 }} sx={{flex: 1, height: '100%', width: '100%', minHeight: '70vh'}}>
      {children}
    </Stack>
    <AdUnit slot="3040504992" style={{height: '100px'}} />
  </Stack>
}