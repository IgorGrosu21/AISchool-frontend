  ...
  return <Stack direction='row' sx={{justifyContent: 'space-between'}}>
    <Stack gap={2}>
      ...
      {progress !== undefined && <Stack direction='row' gap={2} sx={{alignItems: 'center'}}>
        <LinearProgress sx={{flex: 1}} variant='determinate' color='secondary' value={progress ? progress * 100 : 0} />
        <Typography variant='h6' sx={{color: 'secondary.main'}}>{progress ? progress * 100 : 0}%</Typography>
      </Stack>}
    </Stack>
